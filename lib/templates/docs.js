...
var MODEL_SCHEMA = ';ctrl.model.jsonSchema(null, null, '+
  'ctrl.router.option("create", "readonly")' +
')';
var MODEL_SCHEMA_ALLFIELDS = ';ctrl.model.jsonSchema(' +
  'Object.keys(ctrl.model.schema.paths).filter(p=>p!="__v"&&p!="_id")' +
  ', null, ' +
  'ctrl.router.option("update", "readonly")' +
')';
...
controllers.create = {
  ...
  parameters: [{
    ...
    'x-schema-name': '`${ctrl.router.model.modelName}`',
    schema: MODEL_SCHEMA,
    required: true
  }],
  responses: {
...
controllers.update = {
  ...
  parameters:  [{
    ...
    name: '`${ctrl.router.nameSingle}`',
    in: '`${ctrl.method==="get"?"query":"body"}`',
    'x-schema-name': '`${ctrl.router.model.modelName}_Update`',
...
controllers.callInstanceMethod = {
  tags: ['`${ctrl.router.nameSingle}`'],
  operationId: '`${ctrl.exposedMethod.replace(/\\./g, "_")}`',

...
function listQueryFields(ctrl) {
  let queryFields = ctrl.router.option('search', 'queryFields');
  let fields;
  if (queryFields) {
    fields = Object.keys(queryFields).filter(f => queryFields[f]);
  } else {
    queryFields = {};
  ...
  return fields.reduce(
    (list, f) => {
      let path = ctrl.model.schema.paths[f];
      let schema = path.jsonSchema();
      if (!schema.type) return list;
      ...
      if (typeof queryFields[f] === 'object') {
        Object.keys(queryFields[f]).forEach(
          (suffix) => queryFields[f][suffix] && ((suffix === 'eq') ?
            list.push(param) :
            list.push(cloneParam(param, suffix))
        ));
        return list;
      }
      ...
      if (path.instance === 'ObjectID' || path.caster && path.caster.instamce === 'ObjectID') {
        ['ne', 'in', 'nin', 'ex'].forEach(
